class BoardClass:
    ...
    def setUsername(self, player_numer: int, username: str) -> None:
        """Sets the players' usernames and the last player name (defaults to p1)"""
        if (player_numer == 1):
            self.p1_username = username
            self.last_player = username
    ...
    def updateGameBoard(self, p_number: int, p_move: tuple[int, int], current_board: str) -> None:
    ...
    def getStats(self, p_number: int) -> None:
        """Prints the player's statistics."""
        if (p_number == 1):         
            return (f"Username: {self.p1_username} | Last Move: {self.last_player}"
                    f"\nNumber of games played: {self.games_played} | Number of wins: {self.p1_wins}"
                    f"\nNumber of losses: {self.p1_losses} | Number of ties: {self.p1_ties}")
            
        elif (p_number == 2):
            return (f"Username: {self.p2_username} | Last Move: {self.last_player}"
                    f"\nNumber of games played: {self.games_played} | Number of wins: {self.p2_wins}"
                    f"\nNumber of losses: {self.p2_losses} | Number of ties: {self.p2_ties}")
